  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Expandable Menu Links on Mobile */}
        <div 
          ref={mobileMenuRef} 
          className="h-0 opacity-0 overflow-hidden px-1"
        >
          <div className="pt-2.5 pb-2.5 border-t border-white/10 flex flex-col gap-2">
            {/* Links */}
            {MENU_LINKS.map((link) => (
              <Link
                key={link.name}
              ...
              >
                <div className="flex-1 h-11 px-3.5 bg-white/[0.04] active:bg-white/10 border border-white/10 rounded-[10px] flex items-center justify-between text-white transition-all duration-300">
                  <div className="flex items-center gap-3">
                    <div className="w-7 h-7 bg-white/10 rounded-[6px] flex items-center justify-center text-white/90 shrink-0">
                      {link.icon}
                    </div>
                    <span className="font-display text-[13px] uppercase tracking-wider font-medium text-white/90">
                      {link.name}
                    </span>
                  </div>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="opacity-40 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all">
                    <polyline points="9 18 15 12 9 6" />
                  </svg>
                </div>
              </Link>
            ...
            {/* Bottom Row inside Mobile Menu Drawer: CTA & Socials */}
            <div className="menu-item-link pt-2 border-t border-white/10 flex items-center justify-between gap-2 opacity-0">
              <CTAButton text="Termin buchen" href="/contact" variant="primary" />
              
              <div className="flex items-center gap-1.5 bg-white/5 border border-white/10 p-1 rounded-[8px]">
                <a 
                  href="https://wa.me/49123456789" 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="w-8 h-8 flex items-center justify-center rounded-[6px] bg-white/5 active:bg-[#ff3131] text-white/80"
                  aria-label="WhatsApp"
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414-.074-.124-.272-.198-.57-.347m-5.421 7.461c-1.826 0-3.626-.491-5.2-1.42l-.373-.222-3.865 1.013 1.031-3.766-.244-.389c-1.024-1.631-1.564-3.518-1.564-5.457 0-5.696 4.635-10.33 10.334-10.33 2.76 0 5.353 1.077 7.305 3.031 1.952 1.954 3.028 4.548 3.028 7.308 0 5.697-4.636 10.331-10.333 10.331"/></svg>
                </a>
                <a 
